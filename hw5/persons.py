...
if name in persons:
    db.execute('update persons set age = ?, profession = ? where name = ?', (age, profession, name))  # do update
else:
    db.execute('insert into persons values(?,?,?)', (name, age, profession))  # do insert
db.commit()
